...
4. **Tests** — `tests/Rules/<Name>RuleTest.php` extends `AbstractTestCase`, asserts `[message, line]` pairs against `tests/Rules/Data/<Name>/code.php`.
   ...
   - Keep cases that a rule intentionally stays silent on (an ambiguous type it can't resolve, a check it deliberately leaves to PHPStan's native type checking, etc.) out of the `Invalid*` fixture class and into their own separate class named `Skipped<Reason>` (e.g. `SkippedDynamicComponent`, `SkippedUnknownClassComponent`) — mixing silent cases into `Invalid*` makes it unclear, on a later re-read, whether a line producing no error is intentional or a coverage gap.
