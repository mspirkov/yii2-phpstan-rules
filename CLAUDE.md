...
- **Reuse Analyzers, don't reimplement them.** Before writing any type-check, class-check, or config-parsing logic in a rule, check whether `Analyzers/*` already does it — and if you're about to write something a second rule could plausibly need, put it in an `Analyzer` (constructor-injected, autowired via `rules.neon`) instead of inlining it. This project has been bitten by the same "is this class X or a subclass" / "is this expression an instance of X" logic being hand-rolled per rule before `ExpressionTypeAnalyzer` existed; don't regress that. What's already there:
  - `ExpressionTypeAnalyzer` — `isClassNameOf()` / `isClassReflectionOf()` (class-string or `ClassReflection` is-or-extends a given class), `isObjectOf()` (expression's type is-or-extends a class), `getSingleClassReflectionOf()` (the expression's concrete `ClassReflection` if it unambiguously is-or-extends a given class, `null` on zero/multiple matches), `isDefinitelyNotString()` / `isDefinitelyNotStringOrArrayOfStrings()` / `isDefinitelyNotArrayOrObjectOf()`, `hasClass()`.
  ...
  - `BaseObjectPropertyAnalyzer` — does a Yii `BaseObject`/model class have a given property (declared, PHPDoc `@property`, or getter/setter), including the "does this look like a real property name" heuristic for free-form config keys. `hasProperty()` checks mere existence, `hasWritableProperty()` requires a settable target (config assignment), `hasReadableAndWritableProperty()` requires both (a two-way-bound form field) — pick the one matching what the call site actually does with the property, not just the loosest one that compiles.
