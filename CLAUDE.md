...
- **Don't duplicate errors PHPStan already reports natively.** Before adding a check, verify with a scratch `phpstan analyse` run (real signature, real call site) that PHPStan's own type checking doesn't already flag it — many Yii2 APIs (e.g. `Yii::createObject()`) carry accurate `@param class-string<T>|array{...}|callable(): T` PHPDoc, so passing a wrong type, an unknown class-string, or even an unrefined non-`class-string` variable already produces a native `argument.type` / `argument.templateType` error on its own. A rule should only add messages for gaps PHPStan's own type system can't see — e.g. an open array shape (`array{class?: ..., ...}`) type-checks fine with the `class`/`__class` key missing entirely (PHPStan can only report a cryptic "unable to resolve the template type" note), and it never validates the meaning of the other, un-typed keys. When in doubt, write the suspect call in a throwaway file and run `phpstan analyse` with `bootstrapFiles: [tests/bootstrap.php]` (needed for the global `Yii` class to resolve — see below) before deciding whether a check earns its own message.
